...
type Kline struct {
	...
	OpenAt     types.Time `xorm:"notnull timestamp unique(open_at)" json:"open_at"`            //开盘时间
	...
	CloseAt    types.Time `xorm:"timestamp notnull default CURRENT_TIMESTAMP" json:"close_at"` // 收盘时间
...
func (k *Kline) Save() error {
	...
	old := Kline{}
	exist, _ := db.Table(table).Where("open_at=?", time2str(time.Time(k.OpenAt))).Get(&old)
	if !exist {
		_, err := db.Table(table).Insert(&k)
		return err
	} else {
		_, err := db.Table(table).Where("open_at=?", time2str(time.Time(k.OpenAt))).Update(k)
		return err
	}
}
